...
/** Fecha civil hoy en Paraguay (DMH), YYYY-MM-DD. */
function todayIsoAsuncion() {
...
async function syncParaguayToDb() {
  ...
  return withSingleFlight(SOURCE, async () => {
    ...
    if (data.items && data.items.length > 0) {
      ...
      const today = todayIsoAsuncion();
      const fechas = new Set(
        data.items
          .map((row) => fechaDmYToIso(row.fecha))
          .filter(Boolean)
      );
      if (!fechas.has(today)) {
        warnings.push(
          `DMH aún no publicó la fecha de hoy (${today}). Fechas en página: ${[...fechas].join(", ") || "(ninguna)"}. El UPSERT actualizó días previos; reintentá más tarde.`
        );
      }
    }
    try {
